    ...
    def load_data(self, n, dataset, pn, datapath):
        if dataset == 'University Course Dataset':
            ...
            self.x_train, self.y_train, all_concepts = self.get_gt(
                '../datasets/University Course Dataset/train_test_5fold/train_'+str(n)+'_index.txt', all_concepts)
            self.x_test, self.y_test, word_inds = self.get_gt(
                '../datasets/University Course Dataset/train_test_5fold/test_'+str(n)+'_index.txt', all_concepts)
            ...
            all_concepts = set()
            self.x_train, self.y_train, all_concepts = self.get_gt(
                '../datasets/NPTEL MOOC Dataset/train_test_5fold/train_'+str(n)+'_index.txt', all_concepts)
            self.x_test, self.y_test, word_inds = self.get_gt(
                '../datasets/NPTEL MOOC Dataset/train_test_5fold/test_'+str(n)+'_index.txt', all_concepts)
            self.data_path = datapath + str(pn)+'/final.beta'
            words, n_w = self.topics_per_word(word_inds, self.data_path)
