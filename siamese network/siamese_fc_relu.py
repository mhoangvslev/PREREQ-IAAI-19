...
x1 = tf.compat.v1.placeholder(tf.float32, shape=[None, n_steps, n_inputs]) # placeholder for the first network (concept 1)
x2 = tf.compat.v1.placeholder(tf.float32, shape=[None, n_steps, n_inputs]) # placeholder for the second network (concept 2)
...
correct_pred = tf.equal(tf.argmax(input=prediction, axis=1), y) 
...
for n in range(n_runs):
	...
	for n_preq in a:
		...
		with tf.compat.v1.Session() as sess:
			...
			tf.compat.v1.summary.scalar("P_50", pat50)
			tf.compat.v1.summary.scalar("P_100", pat100)
